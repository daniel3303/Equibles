...
public class FtdImportServiceFullPipelineTests : IAsyncLifetime {
    ...
    private IServiceScopeFactory CreateScopeFactory() {
        ...
        scopeFactory.CreateScope().Returns(_ => {
            ...
            sp.GetService(typeof(FailToDeliverRepository)).Returns(new FailToDeliverRepository(ctx));
    ...
    public async Task Import_DownloadAndParseAndUpsert_PersistsFailToDeliverAndSeedsCusipOnMatchingStock() {
        ...
        var csv = "SETTLEMENT DATE|CUSIP|SYMBOL|QUANTITY (FAILS)|DESCRIPTION|PRICE\n" +
                  $"{settlementDate:yyyyMMdd}|037833100|AAPL|12345|APPLE INC|187.50\n";

        var secEdgarClient = Substitute.For<ISecEdgarClient>();
        secEdgarClient.DownloadStream(Arg.Any<string>())
        ...
        var ftdRow = await verify.Set<FailToDeliver>()
            .SingleOrDefaultAsync(f => f.CommonStockId == apple.Id && f.SettlementDate == settlementDate);
        ftdRow.Should().NotBeNull(
        ...
        reloadedApple.Cusip.Should().Be("037833100",
