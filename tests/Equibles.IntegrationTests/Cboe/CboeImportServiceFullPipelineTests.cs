...
public class CboeImportServiceFullPipelineTests : IAsyncLifetime {
    ...
    private IServiceScopeFactory CreateScopeFactory() {
        ...
        scopeFactory.CreateScope().Returns(_ => {
            ...
            sp.GetService(typeof(CboePutCallRatioRepository)).Returns(new CboePutCallRatioRepository(ctx));
            sp.GetService(typeof(CboeVixDailyRepository)).Returns(new CboeVixDailyRepository(ctx));
    ...
    public async Task Import_DownloadsAndPersistsBothPutCallRatiosAndVixHistoryToRealPostgres() {
        ...
        client.DownloadPutCallRatios(CboePutCallCsvType.Equity).Returns([
        ...
        var sut = new CboeImportService(
            ...
            // is enough here.
            new ErrorReporter(Substitute.For<IServiceScopeFactory>(), Substitute.For<ILogger<ErrorReporter>>()));

        await sut.Import(CancellationToken.None);

        await using var verify = _fixture.CreateDbContext();

        var persistedEquity = await verify.Set<CboePutCallRatio>()
            .Where(r => r.RatioType == CboePutCallRatioType.Equity)
            .OrderBy(r => r.Date)
            .ToListAsync();
        persistedEquity.Should().HaveCount(2, "both Equity records should round-trip through FlushPutCallBatch");
        ...
        var otherTypeCount = await verify.Set<CboePutCallRatio>()
            .Where(r => r.RatioType != CboePutCallRatioType.Equity)
            .CountAsync();
