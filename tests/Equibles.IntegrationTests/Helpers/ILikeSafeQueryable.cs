...
{
    public ILikeSafeQueryable(IEnumerable<T> enumerable)
        : base(enumerable) { }

    public ILikeSafeQueryable(Expression expression)
        : base(expression) { }
...
file sealed class ILikeRewriter : ExpressionVisitor
{
    ...
    protected override Expression VisitMethodCall(MethodCallExpression node)
    {
        if (node.Method.Name == "ILike" && node.Arguments.Count >= 3)
        {
            ...
            var stripMethod = typeof(ILikeRewriter).GetMethod(
                nameof(StripWildcards),
