...
public class CboeToolsTests : ParadeDbMcpTestBase {
    ...
    public async Task GetPutCallRatios_ReturnsAscendingDateTableForRequestedType() {
        var equityFirst = new CboePutCallRatio {
            RatioType = CboePutCallRatioType.Equity, Date = new DateOnly(2026, 4, 1),
            CallVolume = 1_200_000, PutVolume = 800_000, TotalVolume = 2_000_000, PutCallRatio = 0.67m,
        };
        var equitySecond = new CboePutCallRatio {
            RatioType = CboePutCallRatioType.Equity, Date = new DateOnly(2026, 4, 2),
            CallVolume = 1_500_000, PutVolume = 1_100_000, TotalVolume = 2_600_000, PutCallRatio = 0.73m,
        };
        var indexNoise = new CboePutCallRatio {
            RatioType = CboePutCallRatioType.Index, Date = new DateOnly(2026, 4, 1),
            CallVolume = 9_999, PutVolume = 9_999, TotalVolume = 19_998, PutCallRatio = 1.0m,
        };
        DbContext.Set<CboePutCallRatio>().AddRange(equityFirst, equitySecond, indexNoise);
        await DbContext.SaveChangesAsync();

        var result = await Sut().GetPutCallRatios(
            type: "equity", startDate: "2026-03-01", endDate: "2026-04-30");
    ...
    public async Task GetPutCallRatios_MaxResultsLimitsRows() {
        var rows = Enumerable.Range(1, 5).Select(i => new CboePutCallRatio {
            RatioType = CboePutCallRatioType.Total, Date = new DateOnly(2026, 4, i),
            CallVolume = 1_000_000 * i, PutVolume = 500_000 * i,
            TotalVolume = 1_500_000 * i, PutCallRatio = 0.5m,
        });
        DbContext.Set<CboePutCallRatio>().AddRange(rows);
        await DbContext.SaveChangesAsync();

        var result = await Sut().GetPutCallRatios(
            type: "Total", startDate: "2026-04-01", endDate: "2026-04-30", maxResults: 2);
    ...
    public async Task GetPutCallRatios_NullVolumes_RenderEmDash() {
        DbContext.Set<CboePutCallRatio>().Add(new CboePutCallRatio {
            RatioType = CboePutCallRatioType.Vix, Date = new DateOnly(2026, 4, 1),
            CallVolume = null, PutVolume = null, TotalVolume = null, PutCallRatio = null,
        });
        await DbContext.SaveChangesAsync();

        var result = await Sut().GetPutCallRatios(
            type: "Vix", startDate: "2026-03-01", endDate: "2026-04-30");
    ...
    public async Task GetVixHistory_ReturnsOhlcTableSortedAscending() {
        var first = new CboeVixDaily {
            Date = new DateOnly(2026, 4, 1), Open = 14.20m, High = 15.30m, Low = 13.80m, Close = 14.95m,
        };
        var second = new CboeVixDaily {
            Date = new DateOnly(2026, 4, 2), Open = 14.95m, High = 16.10m, Low = 14.70m, Close = 15.55m,
        };
    ...
    public async Task GetVixHistory_DateRangeExcludesOutsideRows() {
        DbContext.Set<CboeVixDaily>().AddRange(
            new CboeVixDaily { Date = new DateOnly(2026, 1, 15), Open = 1m, High = 1m, Low = 1m, Close = 1m },
            new CboeVixDaily { Date = new DateOnly(2026, 4, 15), Open = 20m, High = 21m, Low = 19m, Close = 20.5m });
        await DbContext.SaveChangesAsync();
