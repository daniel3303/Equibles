...
public class HoldingsImportServiceTests {
    ...
    // ── IsValueAlreadyInDollars ──

    [Fact]
    public void IsValueAlreadyInDollars_DollarFilerInThousandsDataset_ReturnsTrue() {
        // Median raw price ~0.137 (most filers report in thousands for a $137 stock)
        // This filer reported 107,796,594 VALUE for 786,492 shares → raw price = 137.06 (in dollars)
        var stockId = Guid.NewGuid();
        var reportDate = new DateOnly(2020, 3, 31);
        var consensus = new Dictionary<(Guid, DateOnly), decimal> {
            [(stockId, reportDate)] = 0.137m
        };

        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, stockId, reportDate, rawValue: 107_796_594, shares: 786_492);

        result.Should().BeTrue();
    }

    [Fact]
    public void IsValueAlreadyInDollars_NormalThousandsEntry_ReturnsFalse() {
        // Vanguard: VALUE=2,654,818 (in thousands) for 19,369,747 shares → raw price = 0.137
        var stockId = Guid.NewGuid();
        var reportDate = new DateOnly(2020, 3, 31);
        var consensus = new Dictionary<(Guid, DateOnly), decimal> {
            [(stockId, reportDate)] = 0.137m
        ...
        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, stockId, reportDate, rawValue: 2_654_818, shares: 19_369_747);

        result.Should().BeFalse();
    }

    [Fact]
    public void IsValueAlreadyInDollars_NoConsensusForStock_ReturnsFalse() {
        var consensus = new Dictionary<(Guid, DateOnly), decimal>();

        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, Guid.NewGuid(), new DateOnly(2020, 3, 31), rawValue: 107_796_594, shares: 786_492);

        result.Should().BeFalse();
    }

    [Fact]
    public void IsValueAlreadyInDollars_ZeroShares_ReturnsFalse() {
        var stockId = Guid.NewGuid();
        var reportDate = new DateOnly(2020, 3, 31);
        var consensus = new Dictionary<(Guid, DateOnly), decimal> {
            [(stockId, reportDate)] = 0.137m
        ...
        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, stockId, reportDate, rawValue: 107_796_594, shares: 0);

        result.Should().BeFalse();
    }

    [Fact]
    public void IsValueAlreadyInDollars_NegativeShares_ReturnsFalse() {
        var stockId = Guid.NewGuid();
        var reportDate = new DateOnly(2020, 3, 31);
        var consensus = new Dictionary<(Guid, DateOnly), decimal> {
            [(stockId, reportDate)] = 0.137m
        ...
        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, stockId, reportDate, rawValue: 107_796_594, shares: -100);

        result.Should().BeFalse();
    }

    [Fact]
    public void IsValueAlreadyInDollars_NegativeRawValue_ReturnsFalse() {
        var stockId = Guid.NewGuid();
        var reportDate = new DateOnly(2020, 3, 31);
        var consensus = new Dictionary<(Guid, DateOnly), decimal> {
            [(stockId, reportDate)] = 0.137m
        ...
        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, stockId, reportDate, rawValue: -500, shares: 786_492);

        result.Should().BeFalse();
    }

    [Fact]
    public void IsValueAlreadyInDollars_SlightlyAboveMedian_ReturnsFalse() {
        // 10x above median but below the 100x threshold — could be a high-priced stock
        var stockId = Guid.NewGuid();
        var reportDate = new DateOnly(2020, 3, 31);
        var consensus = new Dictionary<(Guid, DateOnly), decimal> {
            [(stockId, reportDate)] = 0.137m
        ...
        // raw price = 1.37 → 10x median, not enough to trigger
        var result = HoldingsImportService.IsValueAlreadyInDollars(
            consensus, stockId, reportDate, rawValue: 1_370, shares: 1_000);
