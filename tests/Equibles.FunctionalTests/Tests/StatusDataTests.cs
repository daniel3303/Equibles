...
public class StatusDataTests {
    ...
    public async Task Data_GetWithEmptyDatabase_ReturnsJsonPayloadWithAllDataCountsAndWorkerStatuses() {
        ...
        root.GetProperty("databaseConnected").GetBoolean().Should().BeTrue(
            "the Testcontainers Postgres instance is reachable during the request");
        root.GetProperty("dataCounts").GetProperty("StockCount").GetInt32().Should().Be(0,
            "ResetAndSeedAsync truncates every user table before each test");
        root.GetProperty("workers").GetArrayLength().Should().BeGreaterThan(0,
        ...
        root.GetProperty("totalWorkerCount").GetInt32()
            .Should().Be(root.GetProperty("workers").GetArrayLength(),
