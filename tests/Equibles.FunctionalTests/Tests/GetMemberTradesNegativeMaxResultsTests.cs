...
/// <summary>
/// Adversarial end-to-end test for the GetMemberTrades MCP tool's <c>maxResults</c> parameter.
/// Once the member name resolves, the tool feeds the raw client value straight into EF Core's
/// <c>.Take(maxResults)</c> over an <see cref="IQueryable{T}"/>, so a negative cap becomes a
/// negative SQL <c>LIMIT</c> that PostgreSQL rejects (22023); the resulting exception is
/// swallowed by the tool runner and the caller gets the generic internal-failure sentinel.
...
public class GetMemberTradesNegativeMaxResultsTests
        ...
        IAsyncLifetime
{
    ...
    public GetMemberTradesNegativeMaxResultsTests(McpServerAppFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task GetMemberTrades_NegativeMaxResults_DoesNotSurfaceInternalError()
    {
        ...
        var tool = tools.First(t => t.Name == "GetMemberTrades");
