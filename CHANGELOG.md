...
- The Yahoo price lane owns only the rows it wrote: a series replacement keeps venue-derived bars (copying the fresh adjusted close onto a same-basis bar and deleting one the restated series moved onto another basis), the settled-bar resettle never rewrites a venue bar, and a venue bar written first no longer counts as stored Yahoo history, so the deep backfill still runs.
