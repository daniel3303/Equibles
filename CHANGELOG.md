...
- A market whose directory pass fails waits twice as long before each further try, up to its refresh interval, instead of retrying every fifteen minutes for ever. A source that refuses its own data (BME's continuous market names a company its own details place elsewhere) refuses it again on the next call, and that capture costs one request per listed company. An operator refresh request still runs at once.
