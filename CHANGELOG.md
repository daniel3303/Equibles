...
- The European annual report lane (`Equibles.Integrations.XbrlFilings` plus `EsefReportImportService`): the whole `filings.xbrl.org` filing index is read a page at a time at one request a second, the ESEF rows whose filer is identified by a legal entity identifier we hold are kept, one filing per issuer is chosen for its latest period with the country of its presentation listing breaking the tie, and that annual report is stored as a `Document` of the new `DocumentType.EsefAnnualReport` (reference `{LEI}-{yyyyMMdd}-{CC}`, the report as served in `XbrlContent`, its text in `Content`, no as-filed rendering). Both XBRL parsers carry `ConsolidatedLei` beside `ConsolidatedCik` for the ISO 17442 context scheme and `XbrlFactExtractionService.CanFillConsolidated` accepts an ESEF report whose context legal entity identifier matches the issuer's, so the extraction sweep reads the stored envelope with no change of its own. An issuer that also holds a CIK is left to the SEC lane, whose facts a later-filed European report would otherwise supersede on the readers' filed-date tie-break. The type is hidden from filing lists.
- A London directory adapter (`Equibles.Integrations.Lse`): the exchange publishes its instrument list as a numbered workbook on its document host and its reports page is a script application that names no address, so the newest edition is found by asking that host which numbers exist, walking over the gaps the series has and keeping the highest that answers with a document. `Equibles.Integrations.Lse.Xlsx` reads the little of the Office Open XML spreadsheet format this needs, decompressing only the shares sheet and the workbook's string table, never the other thirteen sheets. The sheet states its own as-at date and how many shares it holds, so a truncated or re-shaped workbook is refused rather than read as a shorter market; the stated LSE market decides the venue (AIM is `AIMX`, the main market and its specialist fund and admission-only segments are `XLON`) and `GBX` maps to sterling quoted in pence. One security can hold several lines on one venue, a currency line per quotation and a line per share register: the line quoted in the venue's own currency is kept, and a security whose lines name no single one is left out rather than resolved by preference. A share line the adapter cannot carry is counted and the count reported in the cycle's log line, because absence from a snapshot withdraws a listing. The `lse` catalog entry becomes a live market on the FCA universe with `XLON` and `AIMX` as its venues, and `SameOriginBinaryReader` in `Equibles.Integrations.Common` is the bounded same-origin read behind the text reader and the workbook download.
- Directory adapters for Nasdaq Stockholm, Helsinki and Copenhagen (`Equibles.Integrations.NasdaqNordic`: the venue's shares screener per market and category, the main-market and First North lists each naming their own venue, the instrument reply confirming each line by its exchange label), BME (`Equibles.Integrations.Bme`: the continuous market's listed-companies reply plus each line's own details, which state the ticker, the issuer code and any further current share class of the issuer) and GPW (`Equibles.Integrations.Gpw`: the main market's continuous-trading and two single-price auction tables, confirmed by the company page). The catalog entries for the five markets now carry a directory source and the venue codes FIRDS files them under (Nasdaq's Nordic@Mid and Auction on Demand segments, Madrid's dark midpoint book); `EquityMarketDirectorySymbol` writes a venue's share-class separator (a space on Nasdaq, a dot on BME and London) as the hyphen the site route, the display symbol and the price provider's symbol use. London moves last in the catalog. `SameOriginTextReader` in `Equibles.Integrations.Common` is the bounded same-origin read the three clients share.
- `Equibles.DelayedTrades`: a venue-pluggable delayed-trades lane over each market's MiFIR post-trade file (`IDelayedTradeSource` in `Equibles.Integrations.DelayedTrades`, the Euronext source under `Equibles.Integrations.Euronext.DelayedTrades`). `DelayedTradeScraperWorker` polls every catalog market with a delayed-trades source inside its own session for `LatestDelayedTrade` (the newest 15-minute-delayed price-forming print per verified listing with the session's open, high, low and volume), settles the previous session into `EquityDailyStockPrice` as venue-owned bars (`SourceTicker` = `MIC:ISIN`, prices in the listing's quotation unit) once per `DelayedTradeImportPartition` marker (refused while no bar has landed; a file the latest marker already describes is recognised by its hash before any parse; an older session than the expected one is treated as a holiday only when no intraday poll saw the expected session), re-derives it once a day, grants one closing poll after the intraday window so the session-complete flag always lands, and records every fetch in `DelayedTradeFileCapture` with its URL, terms URL and hash. Prints are classified by the venue's own MMT flags: the closing auction publishes as mechanism 1 and forms the close, dark reference-price and off-book prints add volume only, cancellations and amendments resolve by trade identifier. `EquityMarketRegistration` gains `DelayedTradesCapturedAt`, `DelayedTradesSessionDate` and `DelayedTradesLastError`; `ErrorSource.DelayedTradeScraper` is new. Migration `AddDelayedTrades` is additive.
- `EquityListing.YahooPriceSyncAttemptedAt`, stamped on every Yahoo chart attempt for a catalog listing, served or thrown, through `EquityListingRepository.StampPriceSyncAttempt` (a row-scoped update, no identity lock).
- `Equibles.Yahoo.Data.Prices.DailyBarGuards`, the write guards every daily-bar writer shares (`MaxPriceValue`, the same-basis close tolerance, `IsValidOhlc`, `IsValidCandle`, `IsSameSplitBasis`, `IsVolumeUpgrade`, `IsSettledDailyBar`), and `VenuePriceSource`, the `MIC:ISIN` source key that marks a venue-derived bar in `EquityDailyStockPrice` with `YahooOwned()` and `VenueOwned()` query filters.
...
- A Nasdaq Nordic line too illiquid for continuous trading is admitted: the instrument reply names the trading model after the exchange ("First North GM Sweden Auction"), which is the same venue, and the confirmation now reads the pair of spellings its category may answer with. Four Stockholm lines were refused in production before this, and the spelling is used on all three growth markets, so more than those four were affected. A mismatch now names the reply and the labels expected of it instead of only saying that they disagree.
- A market whose directory pass fails waits twice as long before each further try, up to its refresh interval, instead of retrying every fifteen minutes for ever. A source that refuses its own data (BME's continuous market names a company its own details place elsewhere) refuses it again on the next call, and that capture costs one request per listed company. An operator refresh request still runs at once, and now runs once per press rather than on every control tick while it stands unserved.
- The Euronext catalog entries carry the venues' own session hours: Lisbon and Dublin open at 08:00 local, Dublin's continuous session ends 16:28 with the auction at 16:30, Oslo's ends 16:20 with the auction at 16:25 (pinned from each market's delayed-trades file).
- Yahoo price capture for a catalog listing that a venue keeps current (a venue-owned bar newer than every Yahoo-owned one) fetches the bounded resettle-and-heal window at most once per `YahooPriceScraper:CoveredListingFetchIntervalHours` (24) instead of an ever-growing range every cycle, and still fetches from the floor while the listing has no Yahoo-owned rows; a catalog listing's first full history is installed only when its first storable bar lies within a week of the chart's first trade date (an unknown first trade date installs as before). US listings are untouched.
- `EquityDailyStockPriceRepository.GetPrimarySeries()` returns US presentation-listing rows only: every issuer-level consumer is a US aggregate and venue listings are read by listing id, so a verified venue presentation no longer contributes its series to US figures, dashboards, screens or forecasts.
- The Yahoo price lane owns only the rows it wrote: a series replacement keeps venue-derived bars (copying the fresh adjusted close onto a same-basis bar and deleting one the restated series moved onto another basis), the settled-bar resettle, the invalid-OHLC repair and the historical-cutoff purge never touch a venue bar, and neither the stored-history check nor the forward sync start counts a venue bar, so a venue bar written first (or written ahead of the feed every session) neither stops the deep backfill nor closes the fetch window. The price of that scope is deliberate: once a venue keeps a listing current, the Yahoo-owned latest date freezes at its last pre-venue session and the lane fetches that listing on every cycle over a growing range (so its split, dividend and quotation-basis capture keeps running); the delayed-trades lane must bound that cadence and range while keeping the floor when no Yahoo-owned row exists.
...
- Move a CUSIP to the co-registered sibling class the SEC's fails file shows it trading under. `FtdImportService.ReconcileSiblingCusipsFromArchive` reads the newest twelve fortnightly files at most once a week and calls `EquityIdentityManager.ReassignSiblingCusip` (Arcadia's share class held its units' CUSIP, Stran's warrant held the common's). The stored-label convergence also moves primary rows onto a retired sibling registered on one 12(b) cover page with the presentation, while the presentation holds a CUSIP of its own (Liberty Broadband's Series A history). `CoverPageRegistration` in `Equibles.Sec.FinancialFacts.Data` is the shared co-registration proof.
- Return a CUSIP held by the wrong class of one issuer to the class the SEC states it for. The live FTD reconcile releases a sibling security's CUSIP when the presentation ticker trades under it and the sibling's own ticker already has a different listed CUSIP (Launch One's unit held its Class A CUSIP), and `FtdImportService.ReconcileRetiredSiblingCusips` swaps a presentation CUSIP back to a retired sibling class from stored SEC evidence (Liberty Broadband's Series C held the Series A CUSIP). Both require the two tickers on one 12(b) cover page, which a renamed predecessor never shares, and neither aliases the sibling's CUSIP into the primary.
...
- Converge stored 13F position listing labels after an issuer's presentation listing or CUSIP claims change. `HoldingLabelConvergenceService` moves a label equal to the presentation ticker to primary and a primary row whose CUSIP now resolves to a still-trading sibling (beside a CUSIP-identified presentation) to that sibling's ticker, never onto an occupied key, and marks the moved quarters for rebuild. Each issuer's identity fingerprint is stored in the new `HoldingLabelConvergenceState` table so positions are reread only after that identity changes. The importer and the pass share one CUSIP precedence (`HoldingCusipResolution`), which now resolves a listing claim naming the presentation ticker to primary.
...
- BME directory capture keeps a share line the venue marks on the line itself. The adapter read a non-empty top-level `active` field as a cancelled line, which is what that field means on an issuer's OTHER lines, where the marker is `C` and always arrives with an exclusion date. On the line itself the marker is `S` and means something else: six of the venue's 123 listed companies carried it and every one of them closed in the last settled session, so five live Madrid companies were being dropped and the sixth only escaped notice because the share gate leaves it out on its instrument classification anyway. The venue's own list and the FIRDS live-share gate decide whether a line is current; the marker is kept as evidence.
