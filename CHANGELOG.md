...
- `Equibles.DelayedTrades`: a venue-pluggable delayed-trades lane over each market's MiFIR post-trade file (`IDelayedTradeSource` in `Equibles.Integrations.DelayedTrades`, the Euronext source under `Equibles.Integrations.Euronext.DelayedTrades`). `DelayedTradeScraperWorker` polls every catalog market with a delayed-trades source inside its own session for `LatestDelayedTrade` (the newest 15-minute-delayed price-forming print per verified listing with the session's open, high, low and volume), settles the previous session into `EquityDailyStockPrice` as venue-owned bars (`SourceTicker` = `MIC:ISIN`, prices in the listing's quotation unit) once per `DelayedTradeImportPartition` marker, re-derives it once a day, and records every fetch in `DelayedTradeFileCapture` with its URL, terms URL and hash. Prints are classified by the venue's own MMT flags: the closing auction publishes as mechanism 1 and forms the close, dark reference-price and off-book prints add volume only, cancellations and amendments resolve by trade identifier. `EquityMarketRegistration` gains `DelayedTradesCapturedAt`, `DelayedTradesSessionDate` and `DelayedTradesLastError`; `ErrorSource.DelayedTradeScraper` is new. Migration `AddDelayedTrades` is additive.
- `EquityListing.YahooPriceSyncAttemptedAt`, stamped on every Yahoo chart request for a catalog listing through `EquityListingRepository.StampPriceSyncAttempt` (a row-scoped update, no identity lock).
