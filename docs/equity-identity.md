...
- An issuer whose reported ISIN total exceeds 2,000 is not enumerated: the capture records only the requested ISIN, which the lookup itself confirmed, plus the reported total, so the sibling-ISIN and embedded-CUSIP merge checks do not run for it and an existing issuer is found by LEI alone.
