...
| **Equity Markets** ([`Equibles.EquityMarkets.*`](../../src/Equibles.EquityMarkets.Data)) | ESMA and FCA FIRDS, Euronext and Xetra instrument directories, GLEIF | `EquityMarketRegistration` (one switch row per catalog market), `FirdsInstrumentRecord` (ISIN × venue equity universe with termination dates), `FirdsImportRun` | `FirdsUniverseWorker`, `EquityMarketDirectoryWorker` (writes verified listings into `Equibles.CommonStocks`) | — |
