...
If a partial name matches several tracked filers, the tool stops and returns candidate CIKs instead of selecting one silently; retry with the intended CIK from `SearchInstitutions`. Other failures mean a name has no match in the tracked filer set or the funds share no 13F quarter. To compare two funds in depth instead, see [ask about portfolio overlap between two funds](how-to-ask-about-fund-overlap.md).
