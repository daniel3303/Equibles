...
- **`GetStockPrices`** — a table of daily bars with one row per trading day: Date, Open, High, Low, Close, and Volume. When the provider-adjusted close differs, the table also includes `Adj Close`. Captured split and cash-dividend changes trigger a full-history reconciliation of the exact listed series. Once pending actions reconcile, use `Adj Close` for total-return analysis; an action can remain pending until a later cycle. Use the bars for charting, trend questions, or as the basis for the [technical indicators](how-to-ask-about-technical-indicators.md) Equibles computes on top of the same history.
