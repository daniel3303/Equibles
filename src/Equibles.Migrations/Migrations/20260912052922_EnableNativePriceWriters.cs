...
{
    /// <inheritdoc />
    public partial class EnableNativePriceWriters : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                CREATE FUNCTION eq_sync_retiring_daily_price() RETURNS trigger LANGUAGE plpgsql AS $body$
                DECLARE
                ...
                BEGIN
                    ...
                    IF FOUND THEN
                        IF NEW."SourceTicker" IS DISTINCT FROM mapping."ListedTicker" THEN
                ...
                END;
                $body$;
                CREATE TRIGGER equity_retiring_daily_price_write AFTER INSERT OR UPDATE OR DELETE ON "EquityDailyStockPrice"
                FOR EACH ROW EXECUTE FUNCTION eq_sync_retiring_daily_price();
                """);
        }
        ...
        {
            throw new System.NotSupportedException("Retiring price readers must be removed before writer synchronization can be retired.");
        }
    }
}
