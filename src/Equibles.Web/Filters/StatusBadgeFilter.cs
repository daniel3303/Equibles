...
public class StatusBadgeFilter : IAsyncActionFilter
{
    ...
    public StatusBadgeFilter(ErrorRepository errorRepository, IConfiguration configuration)
    {
    ...
    )
    {
        if (context.Controller is Controller controller)
        {
            ...
            if (string.IsNullOrEmpty(_configuration["Sec:ContactEmail"]))
                warnings++;
            if (string.IsNullOrEmpty(_configuration["McpApiKey"]))
                warnings++;
            if (string.IsNullOrEmpty(_configuration["Finra:ClientId"]))
                warnings++;
            if (string.IsNullOrEmpty(_configuration["Fred:ApiKey"]))
                warnings++;
            var embeddingEnabled = _configuration.GetValue<bool>("Embedding:Enabled");
            if (!embeddingEnabled || string.IsNullOrEmpty(_configuration["Embedding:BaseUrl"]))
                warnings++;
