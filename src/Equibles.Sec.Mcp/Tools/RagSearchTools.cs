...
public class RagSearchTools {
    ...
    public Task<string> SearchDocuments(
        ...
        }, _logger, "SearchDocuments", $"query: {query}", ReportError);
    }
    ...
    public Task<string> SearchCompanyDocuments(
        ...
        return McpToolExecutor.Execute(async () => {
            ...
            var chunks = await _ragManager.SearchRelevantChunksByCompany(query, ticker, maxResults, parsedType,
                ...
                endDate.HasValue ? DateOnly.FromDateTime(endDate.Value) : null);
            return await _ragManager.BuildContext(chunks);
        }, _logger, "SearchCompanyDocuments", $"ticker: {ticker}, query: {query}", ReportError);
    }
    ...
    public Task<string> SearchDocument(
        ...
        return McpToolExecutor.Execute(async () => {
            ...
            return await _ragManager.BuildContext(chunks);
        }, _logger, "SearchDocument", $"documentId: {documentId}, query: {query}", ReportError);
    }
    ...
    public Task<string> ListCompanyDocuments(
        ...
        }, _logger, "ListCompanyDocuments", $"ticker: {ticker}", ReportError);
    }
