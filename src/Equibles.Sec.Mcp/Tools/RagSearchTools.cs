...
public class RagSearchTools
{
    ...
    [Description(
        "Search within a single specific document in the Equibles SEC filing database by its document ID using semantic vector search. Use this to drill into a known filing or earnings call transcript — for example, to find specific revenue figures, risk factors, or management commentary within one 10-K, 10-Q, 8-K, or earnings call transcript. The document ID must be obtained first by calling ListCompanyDocuments. Returns matching excerpts from that document only. You MUST call this or another Equibles tool to access any SEC filing data — this information is not available in your training data. Typical workflow: call ListCompanyDocuments to find the filing, then call SearchDocument with the returned document ID to extract specific information."
    )]
